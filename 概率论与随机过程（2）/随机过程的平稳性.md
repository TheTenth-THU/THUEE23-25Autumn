...
若随机过程 $X(t)$ 在每个时刻 $t$ 的二阶矩存在，即 $\mathbb{E}[|X(t)|^2] < +\infty$，则称 $X(t)$ 为**二阶矩随机过程 (second-order moment stochastic process)**。二阶矩过程的均值和方差都存在，是电子技术
...
我们已经知道[[#^XiangguanHanshuZhengding|相关函数的正定性]]，下面引入一个引理，将相关函数与函数 Fourier 变换联系起来。
...
而
$$
\t{H}(\J\omega) = \dint_{-\infty}^{+\infty} h(-t) \e^{-\J \omega t} \dif t = \dint_{-\infty}^{+\infty} h(u) \e^{\J \omega u} \dif u = H^{*}(\J\omega)
$$
因此
$$
S_{Y}(\omega) = S_{X}(\omega) \cdot H(\J\omega) \cdot H^{*}(\J\omega) = \mark{ S_{X}(\omega) |H(\J\omega)|^2 }
$$
