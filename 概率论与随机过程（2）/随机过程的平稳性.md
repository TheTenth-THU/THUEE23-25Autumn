...
\mathrm{span}\left\{ X(t) \right\} &\longleftrightarrow L^{2}(\mathbb{R}) 
\end{align}
$$
其中：
...
$$
+ $L^{2}(\mathbb{R})$ 是定义在实数集 $\mathbb{R}$ 上、以复指数函数 $\exp(\J\omega t)$ 为基底的二次可积**函数**空间，其上 $F(\omega)$ 与 $G(\omega)$ 间距离**以 $F_{X}(\omega)$ 定义**为
$$
\begin{align}
...
$$
利用欧拉公式 $|e^{j\theta_1} - e^{j\theta_2}|^2 = 2 - 2\cos(\theta_1 - \theta_2)$：

$$\frac{1}{2\pi} \int_{-\infty}^{+\infty} |e^{j\omega t_1} - e^{j\omega t_2}|^2 S_X(\omega) \mathrm{d}\omega = \frac{1}{2\pi} \int_{-\infty}^{+\infty} [2 - 2\cos(\omega(t_1-t_2))] S_X(\omega) \mathrm{d}\omega$$

$$= 2 \cdot \underbrace{\frac{1}{2\pi} \int S_X(\omega) \mathrm{d}\omega}_{R_X(0)} - 2 \cdot \underbrace{\frac{1}{2\pi} \int \cos(\omega(t_1-t_2)) S_X(\omega) \mathrm{d}\omega}_{R_X(t_1-t_2)}$$
